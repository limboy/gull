...
Folder rows have no close button: a row mirrors a file on disk, so removing the folder is the only way to unlist it. Legacy ad-hoc rows keep their close buttons. Rows show a static book icon rather than a cover thumbnail, so a folder of hundreds of books costs no zip reads and nothing image-sized is cached in `localStorage`. Marking a book as finished replaces that row's `book-text` icon with Lucide's `book-check` icon.

...
const MAX_COVER_SIZE = 32 * 1024 * 1024;
...
function readPackageDocument(zip) {
...
function isImageItem(item) {
  return Boolean(item)
    && (item.mediaType.startsWith('image/') || IMAGE_EXTENSION.test(item.href));
}

/**
 * EPUBs advertise their cover in four different ways depending on the
 * generation of the spec (and how sloppy the producer was), so try each in
 * turn and give up rather than guessing at a random image.
 */
function findCoverEntryPath(zip, { $opf, opfDir, manifest }) {
  const resolve = item => path.posix.normalize(opfDir + item.href);

  // EPUB 3: the manifest item marks itself.
  const byProperty = Object.values(manifest)
    .find(item => item.properties.split(/\s+/).includes('cover-image'));
  if (byProperty) return resolve(byProperty);

  // EPUB 2: <meta name="cover" content="manifest-id" />.
  const metaId = $opf('metadata meta[name="cover"]').attr('content');
  if (metaId && isImageItem(manifest[metaId])) return resolve(manifest[metaId]);

  // Producers that skip the metadata but still name the file "cover".
  const byName = Object.entries(manifest)
    .find(([id, item]) => isImageItem(item) && (/cover/i.test(id) || /cover/i.test(item.href)));
  if (byName) return resolve(byName[1]);

  // Last resort: the guide's cover page, whose first image is the artwork.
  const guideHref = $opf('guide reference[type="cover"]').attr('href');
  if (!guideHref) return null;
  const pagePath = path.posix.normalize(opfDir + guideHref.split(/[?#]/, 1)[0]);
  try {
    const $ = cheerio.load(zip.readAsText(pagePath), { xmlMode: true });
    const source = $('img').attr('src')
      || $('image').attr('xlink:href')
      || $('image').attr('href');
    if (!source || source.startsWith('data:')) return null;
    return path.posix.normalize(path.posix.dirname(pagePath) + '/' + source.split(/[?#]/, 1)[0]);
  } catch {
    return null;
  }
}
...
/**
 * Reads just the cover image out of an EPUB. The sidebar needs artwork for
 * books nobody opened yet, and a full parse of every listed book would be
 * orders of magnitude more work than reading one zip entry.
 */
function parseEpubCover(epubPath) {
  const zip = new AdmZip(epubPath);
  assertReasonableArchive(zip);
  const coverPath = findCoverEntryPath(zip, readPackageDocument(zip));
  if (!coverPath) return null;

  const entry = zip.getEntry(coverPath);
  if (!entry || Number(entry.header?.size || 0) > MAX_COVER_SIZE) return null;
  const data = zip.readFile(entry);
  if (!data || data.length === 0) return null;
  return { mime: getImageMime(coverPath), data };
}

function parseEpub(epubPath) {
  const zip = new AdmZip(epubPath);
  assertReasonableArchive(zip);
  const { $opf, opfDir, manifest } = readPackageDocument(zip);
  ...
  for (const idref of spine) {
    ...
    $('link[rel="stylesheet"]').each((_, element) => {
      const href = $(element).attr('href');
      if (!href) return;
      // Some EPUBs link Adobe Digital Editions page-template XML
      // (type="application/adobe-page-template+xml") with rel="stylesheet".
      // It isn't CSS; feeding its XML into the regex-based CSS filter can
      // corrupt the selector of whatever real rule follows it.
      const type = ($(element).attr('type') || 'text/css').toLowerCase();
...
module.exports = { parseEpub, parseEpubCover };
